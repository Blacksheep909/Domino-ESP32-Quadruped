...
| Separate but compatible gait labs | Simulation experiments and LIVE drafts are separate while sharing versioned JSON profiles; LIVE includes a per-leg IK inspector for foot targets, solve validity, commanded joint deltas, limit contact, and stance/swing state; an unavailable, unreachable, or clipped preview blocks draft apply; robot writes remain disarmed-only with two-slot NVS rollback. | Gait, companion, and SIL tests; both ESP32 builds. |
...
The complete standalone/state suite currently contains 138 passing tests. The
